...
@Component({
  selector: 'app-tab-notification-list-sent-tenant',
  templateUrl: './tab-notification-list-sent-tenant.component.html',
  styleUrls: ['./tab-notification-list-sent-tenant.component.scss']
})
export class TabNotificationListSentTenantComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activeRouter.params.subscribe((res: any) => {
      if (res.id) {
        ...
        this.router.navigate(['/tenant/notification']).then(r => []);
      }
    });
  }
