...
@Component({
  ...
  templateUrl: './notification-list-tenant.component.html',
  styleUrls: ['./notification-list-tenant.component.scss', '../../helper.scss']
})
export class NotificationListTenantComponent implements OnInit {
  ...
  getDataGeneralList(){
    const serviceSendingScope = this.notificationService.getSendingScopesList();
    const serviceCreatedBy = this.notificationService.getObjectsCreatedList();
    forkJoin(serviceCreatedBy, serviceSendingScope).subscribe((ref: any) => {
        this.dataCreatedBy = ref[0].data;
        const data = [];
        ref[1].data.forEach(item => {
  ...
  getDataNotification() {
    this.isLoading = true;
    this.notificationService.getNotificationList(this.keyWord, this.valueSendingScope, this.valueRecipientGroup, this.valueCreatedBy, this.valueStatus, this.valueFromTime, this.valueToTime, this.pageIndex, this.pageSize).subscribe((res: any) => {
