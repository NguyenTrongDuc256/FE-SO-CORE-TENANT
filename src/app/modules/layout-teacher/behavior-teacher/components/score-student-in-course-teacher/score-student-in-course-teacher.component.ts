...
import {StudentInfoAPI} from "../../../../../_models/layout-teacher/behavior/score-behavior-teacher.model";
import {
...
@Component({
  selector: 'app-score-student-in-course-teacher',
  templateUrl: './score-student-in-course-teacher.component.html',
  styleUrls: ['./score-student-in-course-teacher.component.scss', '../../style.scss']
})
export class ScoreStudentInCourseTeacherComponent implements OnInit {
  /* component này dùng chung cho chấm điểm cộng và điểm trừ 1 hs lớp môn học */
  ...
  ngOnInit(): void {
    ...
    if (this.type && this.gradeId && this.courseId && this.userId) {
  ...
  handleForkJoinAPI() {
    ...
    const APIGetBehaviorByCourse = this.behaviorTeacherService.getBehaviorForCourse(this.courseId, this.userId, this.type);
    forkJoin([APIGetStudentInfo, APIGetBehaviorByCourse]).subscribe(
      (res: any) => {
  ...
  redirectURLMark(type: 1 | 2) {
    ...
    if (this.type && this.courseId && this.userId) {
      this.handleForkJoinAPI();
    }
  }
