...
} from "../../modals/notification-review-content-staff/notification-review-content-staff.component";
...
@Component({
  ...
  templateUrl: './notification-create-staff.component.html',
  styleUrls: ['./notification-create-staff.component.scss']
})
export class NotificationCreateStaffComponent implements OnInit {
  ...
  openModalReviewNotification() {
    const modalRef = this.modalService.open(NotificationReviewContentStaffComponent, {
      scrollable: true,
  ...
  listenFireBase(action: string, module: string): void {
    ...
    listenFb.subscribe((ref) => {
      if (ref.status) {
        this.isLoading = false;
        this.router.navigate(['/staff/notification/sent']);
      } else {
        this.isLoading = false;
      }
    });
