...
@Component({
  selector: 'app-modal-create-absent-staff',
  templateUrl: './modal-create-absent-staff.component.html',
  styleUrls: ['./modal-create-absent-staff.component.scss']
})
export class ModalCreateAbsentStaffComponent implements OnInit {
  @Input() dataModal: any;
  ...
  getListStudentInClass() {
    this.trainingService.getListStudentInClass(this.dataModal?.dataFromParent).subscribe((res: any) => {
  ...
  createAbsent() {
    ...
    this.trainingService.createAbsent(dataRequest).subscribe((res: any) => {
      this.isLoading = false;
    }, (err: any) => {
      this.isLoading = false;
