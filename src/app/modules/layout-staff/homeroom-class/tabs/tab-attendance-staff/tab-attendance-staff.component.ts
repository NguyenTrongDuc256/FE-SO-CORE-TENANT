...
import {AttendanceStaffService} from "src/app/_services/layout-staff/training/attendance-staff.service";
...
@Component({
  selector: 'app-tab-attendance-staff',
  templateUrl: './tab-attendance-staff.component.html',
  styleUrls: ['./tab-attendance-staff.component.scss']
})
export class TabAttendanceStaffComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.isLoading = true;
    const APIGetConfigAttendance = this.attendanceStaffService.getConfigAttendance();
    const APIGetStudentAndDataAttendanceList = this.attendanceStaffService.getStudentAndDataAttendanceList(this.homeroomClassId, this.attendanceDate, this.keyWord);
    forkJoin([APIGetConfigAttendance, APIGetStudentAndDataAttendanceList]).subscribe(
      (res: any) => {
  ...
  getStudentAndDataAttendanceList(): void {
    ...
    this.attendanceStaffService.getStudentAndDataAttendanceList(this.homeroomClassId, this.attendanceDate, this.keyWord).subscribe(
      (res: any) => {
  ...
  onSubmit(formValue): void {
    ...
    this.attendanceStaffService.saveDataAttendance(dataInput).subscribe((res: any) => {
    }, (_err: any) => {
      this.isLoading = false;
    })
  }
