...
@Component({
  selector: 'app-tab-daily-comment-student',
  templateUrl: './tab-daily-comment-student.component.html',
  styleUrls: ['./tab-daily-comment-student.component.scss']
})
export class TabDailyCommentStudentComponent implements OnInit {
  ...
  getDataDailyComment() {
    ...
    this.dailyCommentStaffService.getInitializationData(this.homeroomClassId, this.currentDateFilter, this.keyWord).subscribe((res: any) => {
  ...
  saveFormDailyComment() {
    ...
    this.listenFireBase("save", "homeroom-class-comment");
    this.saveDataDailyComment(dataInput);
  }

  saveDataDailyComment(dataInput) {
    this.dailyCommentStaffService.saveDailyCommentData(this.homeroomClassId, dataInput).subscribe((res: any) => {
      if (res.status == 0) {
  ...
  openModalImport(): void {
    const modalRef = this.modalService.open(ModalImportDailyCommentStaffComponent,
      {
        scrollable: true,
        windowClass: 'myCustomModalClass',
        keyboard: false,
