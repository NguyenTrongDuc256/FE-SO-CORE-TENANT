...
@Component({
  ...
  styleUrls: ['./score-many-student-in-homeroom-class-staff.component.scss', '../../style.scss']
})
export class ScoreManyStudentInHomeroomClassStaffComponent implements OnInit {
  ...
  onChangeGrade(): void {
    if (this.gradeId) {
      ...
      this.studentList = [];
      this.getHomeroomClassList();
      this.router.navigate(['/staff/behavior/many-student'], {
  ...
  getStudentByHomeroomClassList() {
    ...
    this.behaviorStaffService.getStudentByHomeroomClass(this.homeroomClassId).subscribe(
      (res: any) => {
        clearTimeout(timeoutCallApi);
