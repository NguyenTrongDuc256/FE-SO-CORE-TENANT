...
@Component({
  selector: 'app-score-many-student-in-course-staff',
  templateUrl: './score-many-student-in-course-staff.component.html',
  styleUrls: ['./score-many-student-in-course-staff.component.scss', '../../style.scss']
})
export class ScoreManyStudentInCourseStaffComponent implements OnInit {
  ...
  onChangeGrade(): void {
    if (this.gradeId) {
      ...
      this.studentList = [];
      this.getCourseList();
      this.router.navigate(['/staff/behavior/course/many-student'], {
  ...
  getStudentByCourse() {
    ...
    this.behaviorStaffService.getStudentByCourse(this.courseId).subscribe(
      (res: any) => {
        clearTimeout(timeoutCallApi);
  ...
  getDataConfirm(formValue): void {
    ...
    this.behaviorStaffService.confirmScoreCourse(dataInput).subscribe(
      (res: any) => {
        clearTimeout(timeoutCallApi);
