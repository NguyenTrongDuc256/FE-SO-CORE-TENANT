...
export class AttendanceStaffService {
  ...
  getStudentAndDataAttendanceList(homeroomClassID: string, date: number | string, keyWord: string = '') {
    return this.http.get(
      `${environment.apiStaff}/api/homeroom-class/attendance/staff?homeroomClassId=${homeroomClassID}&date=${date}&keyWord=${keyWord}`
    );
  }
  ...
  saveDataAttendance(data: any) {
    return this.http.post(
      `${environment.apiStaff}/api/homeroom-class/attendance/staff`, data
    );
  }
}
